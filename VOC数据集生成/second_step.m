...
clear;
%注意修改下面四个变量
imgpath='F:\vs程序\faster_rcnn\test\test_faster-rcnn\heying2\';%图像存放文件夹
txtpath='C:\Users\cpeng\Desktop\V000_new.txt';%txt文件
     ...
     [h,w,d]=size(img);
      ...
      pause(0.1);
        ...
        if strcmp(str{1},lastname)%如果文件名相等，只需增加object
           ...
           node=Createnode.createElement('name');
           node.appendChild(Createnode.createTextNode(sprintf('cpeng')));
           owner_node.appendChild(node);
...
for i=1:length(file)
   if length(file(i).name)>=4 && strcmp(file(i).name(end-3:end),'.xml')
    ...
    fnew=fopen(['C:\Users\cpeng\Desktop\Annotations\' file(i).name],'w');
    line=1;
    while ~feof(fold)
        tline=fgetl(fold);
        if line==1
